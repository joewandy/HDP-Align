...
public class Project
{
	...
	public Project(String measurements[], String filenames[], String sets[])
	{
		if (measurements.length != sets.length)
		...
		for (int i=0; i<measurements.length; ++i)
		{
			...
			MeasurementInfo measurementinfo = new MeasurementInfo(i, measurements[i]);
		...
		{
			SetInfo set = header.getSetInfo(sets[i]);
			if (set == null)
			{
				set = new SetInfo(sets[i], SetInfo.SET);
				header.addSetInfo(set);
			}
			
			set.addMeasurementID(i);
		}
	}
	...
	public void writeMeasurements(String path) throws IOException
	{
		for (MeasurementInfo measurement : header.getMeasurementInfos())
		{
			...
			FileOutputStream out = new FileOutputStream(path + "/" + measurement.getLabel() + ".peakml");
