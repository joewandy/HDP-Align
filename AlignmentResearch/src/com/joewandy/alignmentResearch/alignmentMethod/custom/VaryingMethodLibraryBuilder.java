...
public class VaryingMethodLibraryBuilder implements Runnable, PairwiseLibraryBuilder {
	...
	public VaryingMethodLibraryBuilder(BlockingQueue<AlignmentLibrary> queue,
	...
	public AlignmentLibrary producePairwiseLibrary() {
		...
		files.add(data1);
		files.add(data2);
		AlignmentMethod joinAligner = new CustomJoinAlignment(files, massTolerance, rtTolerance);
		AlignmentMethod ransacAligner = new MzMineRansacAlignment(files, massTolerance, rtTolerance);
		AlignmentMethod simaAligner = new SimaAlignment(files, massTolerance, rtTolerance);
		List<AlignmentRow> rows = getAlignedRows(joinAligner);
		rows.addAll(getAlignedRows(ransacAligner));
		rows.addAll(getAlignedRows(simaAligner));
