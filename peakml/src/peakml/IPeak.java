...
public abstract class IPeak extends Annotatable
{
	...
	public static Vector<IPeak> findRelatedPeaks(Vector<IPeak> peaks, double minrt, double rtwindow, RelationCompare<IPeak> compare)
	{
		...
		for (IPeak peak1 : peaks)
		{
			...
			// TODO: joe - quick hack to deal with data that has been combined into a peakset to begin with
			annotateInside(peak1);
	...
	/**
	 * TODO: workaround hack for dealing with peaks inside peakset not annotated correctly
	 * @param peak
	 */
	private static void annotateInside(IPeak peak) {
		if (IPeakSet.class.equals(peak.getClass())) {

			// cast to ipeakset
			IPeakSet<IPeak> ps = (IPeakSet<IPeak>) peak;

			// pass the relation.id and sourcepeakset annotations in this ps to the peaks inside
			int groupId = ps.getAnnotation(Annotation.relationid).getValueAsInteger();
			int sourcePeakSet = ps.getAnnotation(Annotation.sourcePeakset).getValueAsInteger();
			List<IPeak> peaksInside = ps.getPeaks();
			for (IPeak p : peaksInside) {
				p.addAnnotation(Annotation.relationid, Integer.toString(groupId), Annotation.ValueType.INTEGER);					
				p.addAnnotation(Annotation.sourcePeakset, Integer.toString(sourcePeakSet), Annotation.ValueType.INTEGER);					
			}
