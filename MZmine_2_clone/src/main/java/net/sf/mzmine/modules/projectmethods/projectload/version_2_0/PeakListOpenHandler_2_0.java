...
public class PeakListOpenHandler_2_0 extends DefaultHandler implements
	...
	public void startElement(String namespaceURI, String lName, String qName,
		...
		if (qName.equals(PeakListElementName_2_0.PEAK.getElementName())) {
			...
			double rtDouble = Double.parseDouble(attrs.getValue(PeakListElementName_2_0.RT
					.getElementName()));
	...
	public void endElement(String namespaceURI, String sName, String qName)
		...
		if (qName.equals(PeakListElementName_2_0.PEAK.getElementName())) {
			...
			// hack .. hack .. hack .. by joe
			filterNullEntries(dataFile);
	...
	private void filterNullEntries(RawDataFile dataFile) {
		List<Integer> newScanNumbers = new ArrayList<Integer>();
		List<Double> newMasses = new ArrayList<Double>();
		List<Double> newIntensities = new ArrayList<Double>();			
		for (int i = 0; i < numOfMZpeaks; i++) {
			Scan sc = dataFile.getScan(scanNumbers[i]);				
			if (sc == null) {
				continue;
			}
			newScanNumbers.add(scanNumbers[i]);
			newMasses.add(masses[i]);
			newIntensities.add(intensities[i]);
		}
		numOfMZpeaks = newScanNumbers.size();
		scanNumbers = convertIntsToArr(newScanNumbers);
		masses = convertDoublesToArr(newMasses);
		intensities = convertDoublesToArr(newIntensities);
	}
	
	private int[] convertIntsToArr(List<Integer> integers) {
	...
	private double[] convertDoublesToArr(List<Double> doubles) {
